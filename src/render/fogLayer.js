...
export function drawFogOfWar(ctx, world, viewport, fogState = {}) {
...
function clearDiscoveredArea(overlayCtx, world, viewport, discoveredCells) {
  const minX = Math.max(0, Math.floor(viewport.leftWorld) - 1);
  const maxX = Math.min(world.terrain.width - 1, Math.ceil(viewport.leftWorld + viewport.visibleWidth) + 1);
  const minY = Math.max(0, Math.floor(viewport.topWorld) - 1);
  const maxY = Math.min(world.terrain.height - 1, Math.ceil(viewport.topWorld + viewport.visibleHeight) + 1);
...
function drawFogTexture(overlayCtx, seed) {
