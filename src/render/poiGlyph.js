import { normalizePoiMarker } from "../poi/poiModel.js";

export function drawPoiMarkerGlyph(
  ctx,
  x,
  y,
  marker,
  ...
  const normalizedMarker = normalizePoiMarker(marker);
  ...
  drawPoiIcon(ctx, normalizedMarker, x, iconY, glyphScale, {
    highlighted,
    hovered,
    pressed,
  });
...
function drawPoiIcon(ctx, marker, x, y, scale, state) {
  ...
  switch (marker) {
    case "crash-site":
      drawCrashSiteIcon(ctx, x, y, iconScale);
      break;
    case "signpost":
...
function drawCrashSiteIcon(ctx, x, y, scale) {
  ctx.beginPath();
