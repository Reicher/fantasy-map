import { getBiomeRoadTravelCostById } from "../biomes/index";
import { createRng } from "../random";
import { clamp, coordsOf, distance, forEachNeighbor } from "../utils";
...
export function generateRoads(world) {
  ...
  if (seaCandidates.length > 0 && maritimeNodeIds.length >= 2) {
    const seaSelection = selectGraphEdges({
      ...
      loopProbability: 0.36 + density01 * 0.2,
      rng: rng.fork("sea"),
    });
...
function buildLandCandidateEdges({
  ...
  const maxDistance = clamp(11 + nodeSpacing * 2.4 + density01 * 8, 10, 36);
...
function buildSeaCandidateEdges({ settlements, density01 }) {
  ...
  const k = clamp(Math.round(2 + density01 * 2), 2, 5);
  const maxDistance = clamp(24 + density01 * 28, 22, 64);
  const seenPairs = new Set();
  const edges: SettlementPairEdge[] = [];
...
function ensureAtLeastOneLandRoad({
  ...
  const landRoadEdgeSet = new Set();
  const attempts = landCandidates
    .slice()
    .sort(compareEdgeOrder)
    .slice(0, Math.min(8, landCandidates.length));
...
function isMaritimeSettlement(settlement) {
  return Boolean(settlement && (settlement.coastal || settlement.lake || settlement.river));
}
