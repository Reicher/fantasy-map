import { clamp, coordsOf, distance, lerp } from "../utils";
import { describeNode } from "../node/model";

const DEFAULT_NODE_MIN_DISTANCE = 5;
const DEFAULT_ABANDONED_FREQUENCY = 50;

const MIN_SIGNPOST_DEGREE = 3;
const MIN_SIGNPOST_SETTLEMENT_CLEARANCE = 2.2;
...
function buildDedicatedSignpostNodes(world, settlementNodes) {
  ...
  const sourceCells = preferredCells.length
    ? preferredCells
    : [...junctionByCell.keys()];
  const signpostSettlementClearance = Math.max(
    MIN_SIGNPOST_SETTLEMENT_CLEARANCE,
    getEffectiveNodeMinDistance(world.params) * 0.5,
  );
  const forcedCandidates = sourceCells
    ...
    .filter(
      (candidate) =>
        ...
        getNearestPointDistance(candidate.x, candidate.y, settlementNodes) >=
          signpostSettlementClearance,
    )
