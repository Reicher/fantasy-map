...
export function blendStyles(
  ...
  const extraPositive = rng
    .shuffle(secondaryStyle.positiveBlobs)
    .slice(
      0,
      Math.max(1, Math.round(secondaryStyle.positiveBlobs.length * blend)),
    );
  const negativeBlobs = [...primaryStyle.negativeBlobs];
  const extraNegative = rng
    .shuffle(secondaryStyle.negativeBlobs)
    .slice(
      0,
      Math.max(
        1,
        Math.round(Math.max(1, secondaryStyle.negativeBlobs.length) * blend),
      ),
    );
  ...
  return {
    ...
    targetLandRatio:
      primaryStyle.targetLandRatio * (1 - blend * 0.4) +
      secondaryStyle.targetLandRatio * (blend * 0.4),
    seaBias:
      primaryStyle.seaBias * (1 - blend) + secondaryStyle.seaBias * blend,
    noiseScale:
      primaryStyle.noiseScale * (1 - blend) + secondaryStyle.noiseScale * blend,
    mountainBase:
      primaryStyle.mountainBase * (1 - blend) +
      secondaryStyle.mountainBase * blend,
