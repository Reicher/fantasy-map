...
interface SkyStateOptions {
  elapsedHours?: number;
  cloudDriftHours?: number;
  skySeed?: string;
}

const DAY_SKY_TOP_RGB_TUPLE: RgbTriplet = [140, 197, 236];
const DAY_SKY_BOTTOM_RGB_TUPLE: RgbTriplet = [210, 228, 240];
const TWILIGHT_SKY_TOP_RGB_TUPLE: RgbTriplet = [79, 109, 171];
const TWILIGHT_SKY_BOTTOM_RGB_TUPLE: RgbTriplet = [246, 170, 120];
const NIGHT_SKY_TOP_RGB_TUPLE: RgbTriplet = [14, 24, 48];
const NIGHT_SKY_BOTTOM_RGB_TUPLE: RgbTriplet = [49, 70, 104];
...
export function createSkyState(timeOfDayHours, viewW, viewH, options = {}) {
  ...
  const typedOptions = options as SkyStateOptions;
  const elapsedHours = Number.isFinite(typedOptions.elapsedHours)
    ? Math.max(0, typedOptions.elapsedHours ?? 0)
    : hour;
  const cloudDriftHours = Number.isFinite(typedOptions.cloudDriftHours)
    ? Math.max(0, typedOptions.cloudDriftHours ?? 0)
    : elapsedHours;
  const skySeed = hashStringSeed(typedOptions.skySeed ?? "journey-sky");
  ...
  const cloudyDayTop: RgbTriplet = [105, 141, 178];
  const cloudyDayBottom: RgbTriplet = [182, 198, 213];
  const dayTop = lerpRgb(DAY_SKY_TOP_RGB_TUPLE, cloudyDayTop, cloudCover * 0.82);
  const dayBottom = lerpRgb(
    DAY_SKY_BOTTOM_RGB_TUPLE,
    cloudyDayBottom,
    cloudCover * 0.76,
  );
  ...
  const baseTop = lerpRgb(NIGHT_SKY_TOP_RGB_TUPLE, dayTop, daylight);
  const baseBottom = lerpRgb(NIGHT_SKY_BOTTOM_RGB_TUPLE, dayBottom, daylight);

  const dawnTwilightBottom = lerpRgb(TWILIGHT_SKY_BOTTOM_RGB_TUPLE, [255, 142, 108], 0.46);
  const duskTwilightBottom = lerpRgb(TWILIGHT_SKY_BOTTOM_RGB_TUPLE, [236, 132, 104], 0.34);
  ...
  let topRgb = lerpRgb(baseTop, TWILIGHT_SKY_TOP_RGB_TUPLE, twilightTopWeight);
