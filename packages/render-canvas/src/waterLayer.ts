...
import { clamp, coordsOf } from "@fardvag/shared/utils";
...
type WaveNeighborValidator = (nx: number, ny: number) => boolean;
type LakeGeometry = { id: number; loops?: Array<Array<{ x: number; y: number }>> };
...
export function drawLakeWaves(ctx, hydrology, climate, terrain, geometry, viewport, width) {
  const lakeGeometryById = new Map<number, LakeGeometry>((geometry.lakes ?? []).map((lake) => [lake.id, lake]));
...
export function drawOceanWaves(ctx, terrain, climate, _geometry, viewport) {
...
function isWaveLocalPeak(
  kind,
  x,
  y,
  centerScore,
  validNeighbor: WaveNeighborValidator = () => true,
) {
  for (let oy = -1; oy <= 1; oy += 1) {
    for (let ox = -1; ox <= 1; ox += 1) {
      if (ox === 0 && oy === 0) {
