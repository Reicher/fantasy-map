...
export function drawNodes(
  ...
  const visibleIds = new Set(options.visibleNodeIds ?? options.validNodeIds ?? []);
...
function drawNodeTargetHalo(
  ...
  ctx.ellipse(x, haloCenterY, radiusX * 0.71, radiusY * 0.71, 0, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
}

function getNodeZoomScale(viewport: { zoom?: number }): number {
  return Math.max(2.1, Math.min(6.2, (viewport.zoom ?? 1) * 1.32));
}
