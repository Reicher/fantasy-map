...
import { getBiomeDefinitionById } from "@fardvag/shared/biomes";
import { isSnowCell } from "@fardvag/world-gen";
import { coordsOf } from "@fardvag/shared/utils";
import { glyphNoise } from "./hash";

interface ForestRenderOptions {
  showSnow?: boolean;
}

export function collectForestRenderGlyphs(
  world,
  viewport,
  options: ForestRenderOptions = {},
) {
  const showSnow = options.showSnow !== false;
  const regionFeatureById = new Map(
    (
      ((world.features as { biomeRegions?: Array<{ id: number; cells: number[] }> })
        ?.biomeRegions ?? [])
