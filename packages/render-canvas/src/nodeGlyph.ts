import { normalizeNodeMarker } from "@fardvag/shared/node/model";

type NodeGlyphMarker = "settlement" | "signpost" | "abandoned" | "unknown";

interface NodeGlyphState {
  highlighted: boolean;
  hovered: boolean;
  pressed: boolean;
}

interface NodeGlyphOptions extends Partial<NodeGlyphState> {
  scale?: number;
  iconLift?: number | null;
}
...
export function drawNodeMarkerGlyph(
  ...
  const normalizedMarker =
    marker === "unknown"
      ? "unknown"
      : normalizeNodeMarker(marker as NodeGlyphMarker);
...
function drawNodeIcon(
  ctx: CanvasRenderingContext2D,
  marker: NodeGlyphMarker,
  x: number,
  y: number,
  scale: number,
  state: NodeGlyphState,
): void {
